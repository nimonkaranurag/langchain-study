...
* [Customizing agent context](#customizing-agent-context)
* [Tool calling](#tool-calling)
* [Implementation](#implementation)
* [Where to customize](#where-to-customize)
* [Control the input to the subagent](#control-the-input-to-the-subagent)
* [Control the output from the subagent](#control-the-output-from-the-subagent)
* [Handoffs](#handoffs)
...
* Adding human-in-the-loop review for sensitive actions](/oss/python/langchain/supervisor)
...
In **tool calling**, one agent (the “**controller**”) treats other agents as *tools* to be invoked when needed. The controller manages orchestration, while tool agents perform specific tasks and return results. Flow:
...
```
from langchain.tools import tool from langchain.tools import  toolfrom langchain.agents import create_agent from langchain.agents import  create_agent subagent1 = create_agent(model="...", tools=[...]) subagent1 = create_agent(model ="...", tools =[...]) @tool(@tool( "subagent1_name",  "subagent1_name", description="subagent1_description"  description = "subagent1_description"))def call_subagent1(query: str): def  call_subagent1(query: str): result = subagent1.invoke({ result = subagent1.invoke({ "messages": [{"role": "user", "content": query}]  "messages": [{"role": "user", "content": query}] }) }) return result["messages"][-1].content  return result["messages"][- 1].content agent = create_agent(model="...", tools=[call_subagent1]) agent = create_agent(model ="...", tools =[call_subagent1])
```
...
```
from langchain.agents import AgentState from langchain.agents import  AgentStatefrom langchain.tools import tool, ToolRuntime from langchain.tools import tool, ToolRuntime class CustomState(AgentState): class  CustomState(AgentState): example_state_key: str example_state_key: str @tool(@tool( "subagent1_name",  "subagent1_name", description="subagent1_description"  description = "subagent1_description"))def call_subagent1(query: str, runtime: ToolRuntime[None, CustomState]): def  call_subagent1(query: str, runtime: ToolRuntime[None, CustomState]):  # Apply any logic needed to transform the messages into a suitable input  # Apply any logic needed to transform the messages into a suitable input subagent_input = some_logic(query, runtime.state["messages"])  subagent_input = some_logic(query, runtime.state["messages"]) result = subagent1.invoke({ result = subagent1.invoke({ "messages": subagent_input,  "messages": subagent_input, # You could also pass other state keys here as needed. # You could also pass other state keys here as needed.  # Make sure to define these in both the main and subagent's  # Make sure to define these in both the main and subagent's # state schemas. # state schemas. "example_state_key": runtime.state["example_state_key"]  "example_state_key": runtime.state["example_state_key"] }) }) return result["messages"][-1].content  return result["messages"][- 1].content
```
...
```
from typing import Annotated from  typing import  Annotatedfrom langchain.agents import AgentState from langchain.agents import  AgentStatefrom langchain.tools import InjectedToolCallId from langchain.tools import  InjectedToolCallIdfrom langgraph.types import Command from langgraph.types import  Command @tool(@tool( "subagent1_name",  "subagent1_name", description="subagent1_description"  description = "subagent1_description")) # We need to pass the `tool_call_id` to the sub agent so it can use it to respond with the tool call result # We need to pass the `tool_call_id` to the sub agent so it can use it to respond with the tool call resultdef call_subagent1(def  call_subagent1( query: str,  query: str, tool_call_id: Annotated[str, InjectedToolCallId],  tool_call_id: Annotated[str, InjectedToolCallId], # You need to return a `Command` object to include more than just a final tool call # You need to return a `Command` object to include more than just a final tool call) -> Command:) -> Command: result = subagent1.invoke({ result = subagent1.invoke({ "messages": [{"role": "user", "content": query}]  "messages": [{"role": "user", "content": query}] }) }) return Command(update={ return Command(update ={ # This is the example state key we are passing back  # This is the example state key we are passing back "example_state_key": result["example_state_key"],  "example_state_key": result["example_state_key"], "messages": [ "messages": [ ToolMessage( ToolMessage( content=result["messages"][-1].content,  content =result["messages"][- 1].content,  # We need to include the tool call id so it matches up with the right tool call  # We need to include the tool call id so it matches up with the right tool call tool_call_id=tool_call_id  tool_call_id = tool_call_id ) ) ] ] }) })
```

## [​](#handoffs) Handoffs

In **handoffs**, agents can directly pass control to each other. The “active” agent changes, and the user interacts with whichever agent currently has control. Flow:
...
[Human-in-the-loop](/oss/python/langchain/human-in-the-loop)[Retrieval](/oss/python/langchain/retrieval)